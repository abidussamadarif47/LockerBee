...
def aesgcm_encrypt(key: bytes, plaintext: bytes, nonce: bytes, aad: bytes) -> bytes:
    return AESGCM(key).encrypt(nonce, plaintext, aad)

def aesgcm_decrypt(key: bytes, ciphertext: bytes, nonce: bytes, aad: bytes) -> bytes:
...
def gather_files(root: str, encrypt_mode: bool) -> List[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fname in filenames:
            ...
            if is_hidden(fpath):
                continue
            if encrypt_mode and fname.endswith(ENC_EXT):
                continue
            if not encrypt_mode and not fname.endswith(ENC_EXT):
                continue
            files.append(fpath)
    return files
...
class Locker:
    ...
    def lock_file(self, path: str, password: str) -> Tuple[bool, str]:
        try:
    ...
    def unlock_file(self, path: str, password: str) -> Tuple[bool, str]:
        try:
            with open(path, "rb") as f:
                blob = f.read()
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        pad = 10
        frm_top = ttk.LabelFrame(self, text="Target folder")
        ...
        ttk.Radiobutton(frm_mode, text="LOCK", variable=self.mode, value="lock").pack(side="left", padx=(0, 15))
        ttk.Radiobutton(frm_mode, text="UNLOCK", variable=self.mode, value="unlock").pack(side="left")
        ...
        self.append_log("Select folder, enter password, click Start.")
    ...
    def start_task(self):
        ...
        if not root or not os.path.isdir(root):
            messagebox.showerror(APP_NAME, "Select a valid folder.")
            return
        if not pw:
            messagebox.showerror(APP_NAME, "Enter a password/PIN.")
            return
